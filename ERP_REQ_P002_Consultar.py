...
class Consultar(QMainWindow):
    ...
    def Inicio(self):
        ...
        # sqlcabeceraSOLP='''SELECT b.Descrip_Tip_Solp, c.Descripción_Área, d.Nom_usuario, a.Fecha_Solp, a.Estado_Solp, e.Texto FROM TAB_SOLP_001_Cabecera_Solicitud_Pedido a LEFT JOIN `TAB_SOLP_003_Solicitud Pedido` b ON a.Tipo_solp=b.Tipo_Solp LEFT JOIN TAB_SOC_010_Áreas_de_la_empresa c ON a.Area_Solp=c.Cod_Área LEFT JOIN TAB_SOC_005_Usuarios d ON a.Cod_Soc=d.Cod_Soc AND a.Solicita_Solp=d.Cod_usuario LEFT JOIN TAB_SOC_019_Texto_Proceso e ON e.Cod_Soc=a.Cod_Soc AND e.Año=a.Año AND e.Tipo_Proceso='1' AND e.Nro_Doc=a.Nro_Solp AND e.Item_Doc='0' WHERE a.Cod_Soc='%s' AND a.Nro_Solp='%s' AND a.Año='%s';'''%(Cod_Soc,Numero_Solp,Año)
        ...
        # sqlMonto="SELECT SUM(Cant_Mat_Serv*Precio_ref) FROM TAB_SOLP_002_Detalle_Solicitud_Pedido WHERE Cod_Soc='%s'and Nro_Solp='%s'and Año='%s' and Estado_Item!='4';"%(Cod_Soc,Numero_Solp,Año)
        ...
        monto=round(float(listcabecera[6]),2)
        monto="{:,.3f}".format(monto)
        self.leMonto_SOLP.setText(monto)
        self.leMonto_SOLP.setAlignment(QtCore.Qt.AlignRight|QtCore.Qt.AlignTrailing|QtCore.Qt.AlignVCenter)
